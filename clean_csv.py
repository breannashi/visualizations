...
class color_df:
    ...
    def r_unique(self, df, name):
        group_counts = df.groupby(name)[name].transform('count')
        color_indices = np.tile(np.arange(len(self.rgb_colors)), int(np.ceil(len(group_counts)) / len(self.rgb_colors)))[:len(group_counts)]
        df['color'] = np.array(self.rgb_colors)[color_indices].tolist()
        return df
...
class clean_csv:
    ...
    def clean_pytorch(self, df, vis):
        ...
        df['y2']=(df['yc']+0.5*df['h'])
        #fix this in pytorch 
        df=self.col_mean(df, 'sex', 'track_id')
        if vis==True: 
            df=thick_df().r_nothing(df)
            #fix this later
            df=color_df().r_class(df, 'average_sex')
        return df
